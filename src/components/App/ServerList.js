...
const ServerList = ({ onServerClick, onCreateServerClick }) => {
  ...
  return (
    <>
      {servers.map((server) => (
        <TopBarButton key={server.id} picPath={`url(http://localhost/morpwebsite-master/src/pictureData/serverPictures/${server.icon})`} onClick={() => onServerClick(server)} title={server.name} />
      ))}
    </>
  );
};
